...
void UI::Create ()
{
	try
	{
		CreateWindowClass(0, UI::UICallback, "LHack", Module);
		MainWindow = new Window ("LHack", "LHack", WS_EX_CLIENTEDGE | WS_EX_DLGMODALFRAME, WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 300, 100, Module);
...
long __stdcall UI::UICallback (HWND WindowHandle, unsigned int Message, WPARAM wParam, LPARAM lParam)
{
	try
	{
		...
		case WM_CREATE:
		{
			...
			LoadingView = new View (ThisWindow->GetHandle(), 300, 65, "LHack");
			MainView = new View (ThisWindow->GetHandle(), 200, 205, "LHack");
		...
		case WM_CLOSE:
		case WM_DESTROY:
			Notification(ThisWindow->GetHandle(), "You must restart World of Warcraft to unload LHack.");
			return 0;
